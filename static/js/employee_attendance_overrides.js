...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const listBody = document.getElementById("employeeAttendanceOverrideListBody");
  const lockNotice = document.getElementById("employeeAttendanceOverrideLockNotice");
  const editModal = new bootstrap.Modal(document.getElementById("employeeAttendanceOverrideEditModal"));
  const editForm = document.getElementById("employeeAttendanceOverrideEditForm");
  const editBody = document.getElementById("employeeAttendanceOverrideEditBody");
  const editMeta = document.getElementById("employeeAttendanceOverrideEditMeta");
  const editRemark = document.getElementById("employeeAttendanceOverrideEditRemark");
  const editSaveBtn = document.getElementById("employeeAttendanceOverrideEditSaveBtn");
  const updatedAtLabel = document.getElementById("employeeAttendanceOverrideUpdatedAt");
  ...
  function updateEmployeeMetrics(status = null, activeRow = null) {
    ...
    document.getElementById("employeeAttendanceOverrideMetricEmployee").textContent = activeRow
  ...
  function summaryText(values) {
    const parts = employeeAttendanceOverrideFields
      .map(([key, label]) => {
  ...
  function renderListRows() {
    if (!overrideRows.length) {
      listBody.innerHTML = `
        <tr id="employeeAttendanceOverrideTableEmpty">
          <td class="text-muted" colspan="9">当前条件下暂无员工修正数据</td>
        </tr>
      `;
      return;
    }
  ...
  function renderEditForm(row) {
    ...
    editBody.innerHTML = employeeAttendanceOverrideFields
      .map(([key, label]) => {
  ...
  async function loadEmployeeAttendanceOverrideList() {
    ...
    await window.AppQueryProgress.with(listBody, {
      ...
      const [res, accountSet] = await Promise.all([
        fetch(`/admin/employee-attendance-overrides/list?${query.toString()}`),
      ...
      renderListRows();
      updateLockNotice();
      updateEmployeeMetrics(`已查询 ${overrideRows.length} 人`);
    });
  }
  ...
  async function saveEmployeeAttendanceOverride() {
    ...
    const month = monthInput.value;
    if (!empId || !month) {
      window.AppDialog.alert("请先从修正列表选择需要编辑的员工");
      return;
    }
    const payload = {
      month,
    ...
    renderListRows();
    updateEmployeeMetrics("已保存", nextRow);
  ...
  async function importEmployeeAttendanceOverride(file) {
    ...
    if (selectedEmployeeIds().length) {
      await loadEmployeeAttendanceOverrideList();
    }
  }
  ...
  document.getElementById("selectedEmpIds").addEventListener("change", () => updateEmployeeMetrics("等待查询"));
  document
    .getElementById("employeeAttendanceOverrideQueryBtn")
    .addEventListener("click", loadEmployeeAttendanceOverrideList);
  document
  ...
  editSaveBtn.addEventListener("click", saveEmployeeAttendanceOverride);
});
