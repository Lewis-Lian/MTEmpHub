...
export default function MessageCenter() {
  ...
  return <div className="message-center">
    ...
    {isOpen ? <section aria-label="消息中心" className="message-center-panel">
      ...
      <div className="message-center-list">{messages.length ? messages.map((message) => <article className={message.unread ? "is-unread" : ""} key={message.id} onClick={() => void handleRead(message)}><div><strong>{message.title}</strong>{message.unread ? <span className="message-center-dot" /> : null}</div><p>{message.content}</p><small>{message.sender}</small></article>) : <p className="message-center-empty">暂无消息</p>}</div>
    </section> : null}
  </div>;
}
