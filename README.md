...
- **钉钉同步**：可将管理人员考勤来源切换为钉钉，并查看同步历史与未匹配记录。
...
启用钉钉管理人员考勤时，在 `.env` 中填写 `DINGTALK_CLIENT_ID`、
`DINGTALK_CLIENT_SECRET` 和 `DINGTALK_CORP_ID`。这些值只在服务端使用，
不要提交到版本库或写入前端配置。
...
在钉钉开放平台为应用开启**考勤记录读取权限**和**组织通讯录读取权限**（全组织范围的部门和用户信息，
用于按工号解析管理人员的钉钉用户 ID）；应用凭证对应的企业必须与
`DINGTALK_CORP_ID` 一致。管理员在「数据库设置 → 管理人员考勤来源」选择
「钉钉」后，先使用「测试连接」确认凭证和权限（测试连接会依次校验访问令牌、通讯录读取和考勤接口访问），
再在账套中执行同步。

同步按账套月份读取钉钉记录，并按员工工号匹配本系统的管理人员。未填写
`dingtalk_user_id` 的管理人员会通过通讯录按工号自动解析（仅在内存中使用，不回写员工资料）；
无法匹配管理人员的钉钉打卡记录，以及通讯录中无法对应本系统任何员工的工号，会被**跳过**并计入
同步结果列出（可从同步历史下载 CSV），不会静默写入错误员工。来源为
「本地」时，管理人员考勤仍通过原有文件上传流程导入。

选择钉钉来源后，当月存在成功或部分成功的同步时，管理人员统计以每日打卡数据为准，
不再使用旧的 Excel 月度汇总；钉钉模式下未能匹配的管理人员没有考勤数据兜底，请先补齐工号再同步。

钉钉同步所需的后台接口权限仍受管理员登录权限保护；前端只展示同步状态，凭证不会
通过接口返回。
